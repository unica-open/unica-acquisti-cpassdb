...
);e  cpass_t_parametro set valore = 'SEGRETERIA' where ambiente = 'ACTA'
...
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
  ('SOGLIA_DI_NON_INVIO_MIT', '40000', true, '', 'MIT', 'ricerca soggetto fonte esterna ','COTO')
...
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
  ('IMPLEMENTOR_POJO_NAME', 'it.csi.cpass.cpassbe.lib.external.impl.acta.DocumentaleHelperImpl', true, 'DOCUMENTALE', 'ACTA', 'implementatore','COTO')
) AS tmp (chiave, valore, abilitata, riferimento, ambiente, note, ente)
JOIN cpass.cpass_t_ente te ON te.ente_codice = tmp.ente 
WHERE NOT EXISTS (
  SELECT 1
...
FROM ( VALUES
    ...
    ,('IMPLEMENTOR_POJO_NAME', 'it.csi.cpass.cpassbe.lib.external.impl.acta.ProtocolloHelperImpl','DOCUMENTALE', 'ACTA', 'implementatore', true,'COTO')    
