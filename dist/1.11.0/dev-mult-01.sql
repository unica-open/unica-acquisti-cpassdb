...
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
  ('SOGLIA_DI_NON_INVIO_MIT', '40000', true, '', 'MIT', 'ricerca soggetto fonte esterna ','CSI')
...
update cpass_t_parametro set valore = 'it.csi.cpass.cpassbe.lib.external.impl.acta.DocumentaleHelperImpl' 
where chiave = 'IMPLEMENTOR_POJO_NAME' and ambiente = 'ACTA' and riferimento = 'DOCUMENTALE'
and ente_id in(select ente_id from cpass_t_ente where ente_codice ='CSI');

INSERT INTO cpass.cpass_t_parametro (chiave, valore, abilitata, riferimento, ambiente, note, ente_id)
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
  ('IMPLEMENTOR_POJO_NAME', 'it.csi.cpass.cpassbe.lib.external.impl.acta.DocumentaleHelperImpl', true, 'DOCUMENTALE', 'ACTA', 'implementatore','CSI')
) AS tmp (chiave, valore, abilitata, riferimento, ambiente, note, ente)
JOIN cpass.cpass_t_ente te ON te.ente_codice = tmp.ente 
WHERE NOT EXISTS (
  SELECT 1
...
FROM ( VALUES
    ...
    ,('IMPLEMENTOR_POJO_NAME', 'it.csi.cpass.cpassbe.lib.external.impl.acta.ProtocolloHelperImpl','DOCUMENTALE', 'ACTA', 'implementatore', true,'CSI')    
