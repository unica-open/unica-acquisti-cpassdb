...
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM ( VALUES
     ('FORMA_DOC_ORDINI',  '186','DOCUMENTALE', 'ACTA', 'FORMA_DOC_ORDINI', true,'REGP')
    ,('STATO_EFFICACIA',   '8','DOCUMENTALE', 'ACTA', 'STATO_EFFICACIA', true,'REGP')
    ,('VITAL_RECORD_CODE', '12','DOCUMENTALE' , 'ACTA', 'VITAL_RECORD_CODE', true,'REGP')  
) AS tmp(chiave, valore, riferimento, ambiente, note, abilitata,ente)
JOIN cpass_t_ente te ON te.ente_codice = tmp.ente
WHERE NOT EXISTS(
	SELECT 1
...
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
  ('SOGLIA_DI_NON_INVIO_MIT', '40000', true, '', 'MIT', 'ricerca soggetto fonte esterna ','REGP')
...
update cpass_t_parametro set valore = 'it.csi.cpass.cpassbe.lib.external.impl.acta.DocumentaleHelperImpl' where chiave = 'IMPLEMENTOR_POJO_NAME' and ambiente = 'ACTA' and riferimento = 'DOCUMENTALE';
INSERT INTO cpass.cpass_t_parametro (chiave, valore, abilitata, riferimento, ambiente, note, ente_id)
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
  ('IMPLEMENTOR_POJO_NAME', 'it.csi.cpass.cpassbe.lib.external.impl.acta.DocumentaleHelperImpl', true, 'DOCUMENTALE', 'ACTA', 'implementatore','REGP')
) AS tmp (chiave, valore, abilitata, riferimento, ambiente, note, ente)
JOIN cpass.cpass_t_ente te ON te.ente_codice = tmp.ente 
WHERE NOT EXISTS (
  SELECT 1
...
FROM ( VALUES
    ...
    ,('IMPLEMENTOR_POJO_NAME', 'it.csi.cpass.cpassbe.lib.external.impl.acta.ProtocolloHelperImpl','DOCUMENTALE', 'ACTA', 'implementatore', true,'REGP')    
