...
delete from cpass_r_ruolo_modulo where 
ruolo_id in (
select ruolo_id from cpass_d_ruolo where ruolo_codice in ('REFERENTE_RICHIESTA','VALIDATORE_RICHIESTA','GESTORE_MAGAZZINO')
);
...
delete from cpass_d_ruolo where ruolo_codice in ('REFERENTE_RICHIESTA','VALIDATORE_RICHIESTA','GESTORE_MAGAZZINO');

alter table cpass.cpass_r_utente_settore drop constraint if exists idx_utente_settore;


INSERT INTO cpass.cpass_d_ruolo(ruolo_codice, ruolo_descrizione, selezionabile_da_procedura)
SELECT tmp.codice, tmp.descrizione, tmp.selezionabile_da_procedura
FROM (VALUES
	('GESTORE_ODS', 'GESTORE ODS','SI'),
	('LETTORE_ODS', 'LETTORE ODS','SI')
) AS tmp(codice, descrizione, selezionabile_da_procedura)
WHERE NOT EXISTS (
	SELECT 1
...
FROM (VALUES
	('RICERCA_ODS','Ricerca e consulta l''oggetto di spesa','BO',true,'V', 'SI', true),
	('INS_ODS','Inserisce l''oggetto di spesa','BO',true,'V', 'SI', true),
	('MOD_ODS','Modifica l''oggetto di spesa','BO',false,'B', 'SI', true)
...
FROM (VALUES
	('GESTORE_ODS','RICERCA_ODS'),
	('GESTORE_ODS','INS_ODS'),
	('GESTORE_ODS','MOD_ODS'),
	('LETTORE_ODS','RICERCA_ODS'),
	('ADMIN','RICERCA_ODS'),
	('ADMIN','INS_ODS'),
	('ADMIN','MOD_ODS')
...
delete  from cpass_t_parametro where ambiente = 'STILO'
and chiave in(
 'WSDL_LOCATION_RICERCA_DOC'
,'DD'
,'AD'
,'DG'
);
...
delete from cpass_r_ruolo_permesso where permesso_id in(
select permesso_id from cpass_d_permesso where permesso_codice = 'INS_ODS_MAG'

);

DELETE from cpass_d_permesso where permesso_codice = 'INS_ODS_MAG';

DROP FUNCTION if exists cpass.pck_cpass_pba_rep_soggetti_aggregatori(p_programma_id varchar);
DROP FUNCTION if exists cpass.pck_cpass_pba_rep_soggetti_aggregatori_new(p_programma_id varchar);
