...
FROM ( VALUES
	('TITOLARIO_ID', '6','DOCUMENTALE', 'ACTA', 'id titolario', true,'CMTO')
...
--da modificare con i dati di prod
INSERT INTO cpass.cpass_t_parametro (chiave, valore, abilitata, riferimento, ambiente, note, ente_id)
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
  ('WSDL_LOCATION', 'https://tst-aphr01.csi.it:4444/webservices/SOAProvider/plsql/cuscsi_soa_citta_metro_torino/?wsdl', true, 'UTENTE-HR', 'HR', 'location servizio','CMTO'),
  ('USER', 'WS_UNICA_BKOFF_CMTO', true, 'UTENTE-HR', 'HR', 'user','CMTO'),
  ('PW', 'mypass$1', true, 'UTENTE-HR', 'HR', 'pw','CMTO')
