...
ALTER FUNCTION cpass.pck_cpass_ord_rep_ordini(character varying)
    OWNER TO cpass;

GRANT EXECUTE ON FUNCTION cpass.pck_cpass_ord_rep_ordini(character varying) TO cpass_rw;

GRANT EXECUTE ON FUNCTION cpass.pck_cpass_ord_rep_ordini(character varying) TO cpass;

GRANT EXECUTE ON FUNCTION cpass.pck_cpass_ord_rep_ordini(character varying) TO PUBLIC;
...
CREATE OR REPLACE FUNCTION cpass.pck_cpass_ord_rep_ordini_dest(
	p_destinatario_id character varying)
    RETURNS TABLE(ordine_anno integer, ordine_numero integer, ordine_data_emis timestamp without time zone, ordine_consegna_da timestamp without time zone, ordine_consegna_a timestamp without time zone, ordine_num_procedura character varying, ordine_note character varying, ordine_provv_numero character varying, ordine_provv_anno integer, ordine_cons_rif character varying, ordine_consegna_indirizzo character varying, ordine_consegna_cap character varying, ordine_consegna_localita character varying, ordine_tipo_doc character varying, ordine_tipo_proc character varying, ufficio_codice character varying, settore_codice character varying, settore_descrizione character varying, settore_indirizzo character varying, settore_num_civico character varying, settore_cap character varying, settore_localita character varying, settore_provincia character varying, rda_anno integer, rda_numero integer, ente_codice_fiscale character varying, forn_ragione_sociale character varying, forn_indirizzo character varying, forn_numero_civico character varying, forn_cap character varying, forn_comune character varying, forn_provincia character varying, forn_codice character varying, settdest_codice character varying, settdest_descrizione character varying, dest_indirizzo character varying, dest_num_civico character varying, dest_cap character varying, dest_localita character varying, dest_provincia character varying, dest_contatto character varying, dest_email character varying, dest_telefono character varying, ogg_descrizione character varying, ogg_codice character varying, codifica_listino_fornitore character varying, unita character varying, ordine_quantita numeric, ordine_percent_sconto numeric, ordine_percent_sconto2 numeric, aliquota_iva character varying, ordine_prezzo_unitario numeric, ordine_importo_totale numeric, ordine_importo_sconto numeric, ordine_importo_sconto2 numeric, ordine_importo_netto numeric, ordine_importo_iva numeric, rigaordine_note character varying, ordine_consegna_parziale boolean, utente_nome character varying, utente_cognome character varying, utente_telefono character varying, dirigente_nome character varying, dirigente_cognome character varying, anno_protocollo integer, numero_protocollo character varying) 
    LANGUAGE 'plpgsql'
...
ALTER FUNCTION cpass.pck_cpass_ord_rep_ordini_dest(character varying)
    OWNER TO cpass;

GRANT EXECUTE ON FUNCTION cpass.pck_cpass_ord_rep_ordini_dest(character varying) TO cpass_rw;

GRANT EXECUTE ON FUNCTION cpass.pck_cpass_ord_rep_ordini_dest(character varying) TO cpass;

GRANT EXECUTE ON FUNCTION cpass.pck_cpass_ord_rep_ordini_dest(character varying) TO PUBLIC;

--ALTER TABLE cpass.cpass_d_aoo_acta  ALTER COLUMN aoo_acta_orig_id TYPE VARCHAR(50);
--SELECT * FROM CPASS_T_ENTE = '25b3cb3e-0c7e-53b2-88bd-afc47011647d';





-- View: cpass.cpass_v_settore

-- View: cpass.cpass_v_settore

alter table cpass_t_settore add column if not exists data_validita_inizio TIMESTAMP;
alter table cpass_t_settore add column if not exists data_validita_fine TIMESTAMP;

update cpass_t_settore set data_validita_inizio = data_creazione where data_validita_inizio is null;


DROP VIEW if exists cpass.cpass_v_settore;

CREATE OR REPLACE VIEW cpass.cpass_v_settore AS
 WITH RECURSIVE alberosettore AS (
         SELECT 1 AS livello,
            ...
            s.ente_id,
            s.data_validita_inizio,
            s.data_validita_fine,
            ts.tipo_settore_id,
            ts.flag_utilizzabile
           FROM cpass_t_settore s,
            cpass_d_tipo_settore ts
          WHERE 
          s.settore_padre_id IS NULL 
          ...
          --AND (s.data_validita_fine   IS NULL OR s.data_validita_fine   IS NOT NULL AND date_trunc('day'::text, s.data_validita_fine)   > date_trunc('day'::text, now()))
          --AND (s.data_validita_inizio IS NULL OR s.data_validita_inizio IS NOT NULL AND date_trunc('day'::text, s.data_validita_inizio) <= date_trunc('day'::text, now()))
        ...
        UNION ALL
         SELECT mtree.livello + 1,
            ...
            s_figlio.ente_id,
            s_figlio.data_validita_inizio,
            s_figlio.data_validita_fine,
            ts.tipo_settore_id,
            ts.flag_utilizzabile
          ...
          WHERE           
          (s_figlio.data_cancellazione IS NULL   OR s_figlio.data_cancellazione   IS NOT NULL AND date_trunc('day'::text, s_figlio.data_cancellazione)   > date_trunc('day'::text, now()))
          --AND (s_figlio.data_validita_fine IS NULL   OR s_figlio.data_validita_fine   IS NOT NULL AND date_trunc('day'::text, s_figlio.data_validita_fine)   >  date_trunc('day'::text, now()))
          --AND (s_figlio.data_validita_inizio IS NULL OR s_figlio.data_validita_inizio IS NOT NULL AND date_trunc('day'::text, s_figlio.data_validita_inizio) <= date_trunc('day'::text, now()))        
 )
 SELECT row_number() OVER () AS id_v_settore,
    ...
    alberosettore.data_validita_inizio,
    alberosettore.data_validita_fine
  FROM alberosettore
...
AS
 WITH RECURSIVE alberocpvods(livello, cpv_id_padre, cpv_id, cpv_codice,
...
FROM cpass_v_cpv cvc
WHERE (EXISTS (
              SELECT 1
              FROM cpass_d_oggetti_spesa cdos
              WHERE cdos.cpv_id = cvc.cpv_id
              AND (cdos.data_cancellazione IS NULL OR cdos.data_cancellazione IS NOT NULL AND date_trunc('day'::text, cdos.data_cancellazione) > date_trunc('day'::text, now())) 
	))
UNION ALL
SELECT cvc2.livello,
            cvc2.cpv_id_padre,
...
WHERE alberocpvods.cpv_id_padre = cvc2.cpv_id
    ...
    FROM (
        SELECT DISTINCT alberocpvods.livello,
...
-- FUNCTION: cpass.pck_cpass_pba_rep_soggetti_aggregatori(character varying)

-- DROP FUNCTION cpass.pck_cpass_pba_rep_soggetti_aggregatori(character varying);
...
and   1000000 <(select sum (imp.intervento_importi_importo_anno_primo +imp.intervento_importi_importo_anno_secondo +
...
group by inter.intervento_id, inter.programma_id,importiva.iva_primo_anno, 
  importiva.iva_secondo_anno,importiva.iva_anni_successivi),
importi AS (
