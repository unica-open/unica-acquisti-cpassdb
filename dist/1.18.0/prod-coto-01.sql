...
FROM (VALUES
  ('SOGLIA_IVA_OBBLIGATORIA', '0', true, 'PBA', 'CPASS', 'soglia iva obbligatoria ','COTO')
...
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
    ('NSO_SENDER_IDENTIFIER','0201:', true,'NSO', 'NOTIER', 'NSO_SENDER_IDENTIFIER prefix', 'COTO')
) AS tmp (chiave, valore, abilitata, riferimento, ambiente, note, ente)
JOIN cpass.cpass_t_ente te ON te.ente_codice = tmp.ente 
WHERE NOT EXISTS (
  SELECT 1
...
FROM (VALUES
	...
	('RUO','SVINCOLA_CAMPI_EDIT'),
	('RUP','SVINCOLA_CAMPI_EDIT'),
	('DELEGATO_RUP','SVINCOLA_CAMPI_EDIT'),
