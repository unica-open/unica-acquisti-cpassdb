...
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
   ('report.endpoint', 'https://10.138.162.68:10310/cpassrepeng/report', true, '', 'CPASS', 'ricerca soggetto fonte esterna ','CSI')
  ,('report.multi.endpoint', 'https://10.138.162.68:10310/cpassrepeng/multi-report', true, '', 'CPASS', 'ricerca soggetto fonte esterna ','CSI') 
) AS tmp (chiave, valore, abilitata, riferimento, ambiente, note, ente)
JOIN cpass.cpass_t_ente te ON te.ente_codice = tmp.ente 
WHERE NOT EXISTS (
  SELECT 1
