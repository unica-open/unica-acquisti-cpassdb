...
FROM (VALUES
  ('DURATA_PROGRAMMA', 'BIENNALE', true, 'PBA', 'CPASS', 'programma biennale','REGP')
...
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
   ('report.endpoint',       'http://10.138.128.170:10110/cpassrepeng/report'      , true, '', 'CPASS', 'ricerca soggetto fonte esterna ','REGP')
  ,('report.multi.endpoint', 'http://10.138.128.170:10110/cpassrepeng/multi-report', true, '', 'CPASS', 'ricerca soggetto fonte esterna ','REGP') 
) AS tmp (chiave, valore, abilitata, riferimento, ambiente, note, ente)
JOIN cpass.cpass_t_ente te ON te.ente_codice = tmp.ente 
WHERE NOT EXISTS (
  SELECT 1
