...
delete from cpass_t_parametro where chiave in ('report.endpoint','report.multi.endpoint')
INSERT INTO cpass.cpass_t_parametro (chiave, valore, abilitata, riferimento, ambiente, note, ente_id)
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
   ('report.endpoint', 'http://10.138.162.64:10910/cpassrepeng/report', true, '', 'CPASS', 'ricerca soggetto fonte esterna ','CSI')
  ,('report.multi.endpoint', 'http://10.138.162.64:10910/cpassrepeng/multi-report', true, '', 'CPASS', 'ricerca soggetto fonte esterna ','CSI') 
) AS tmp (chiave, valore, abilitata, riferimento, ambiente, note, ente)
JOIN cpass.cpass_t_ente te ON te.ente_codice = tmp.ente 
WHERE NOT EXISTS (
  SELECT 1
