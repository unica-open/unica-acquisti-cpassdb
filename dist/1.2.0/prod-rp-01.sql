...
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
    ('NSO_RECIPIENT_ID','', true,'NSO', 'NOTIER', 'Per integrazione NSO, valorizzare SOLO per ambiente di test o sviluppo. Se lasciato bianco il dato viene valorizzato a runtime.', 'REGP')
) AS tmp (chiave, valore, abilitata, riferimento, ambiente, note, ente)
JOIN cpass.cpass_t_ente te ON te.ente_codice = tmp.ente 
WHERE NOT EXISTS (
  SELECT 1
...
update cpass.cpass_t_parametro set valore = 'https://www.serviziocontrattipubblici.it/WSLogin/rest' where chiave='MIT_URL_WSLOGIN';
update cpass.cpass_t_parametro set valore = 'https://www.serviziocontrattipubblici.it/WSProgrammi/rest' where chiave='MIT_URL_WSPROGRAMMI';
update cpass.cpass_t_parametro set valore = '3128' where chiave='MIT_PROXY_PORT';
update cpass.cpass_t_parametro set valore = 'podto1-proxy.site01.nivolapiemonte.it' where chiave = 'MIT_PROXY_HOSTNAME';
update cpass.cpass_t_parametro set valore = 'RegPiemonteApp' where chiave = 'MIT_CLIENT_ID';
update cpass.cpass_t_parametro set valore = 'M9sA2g$rJXmLCn7h' where chiave = 'MIT_CLIENT_KEY';
...
FROM (VALUES
  ('AMBIENTE','PROD',true,'PROVVEDIMENTO','STILO','ambiente di integrazione')
...
-- configurazione integrazione STILO
INSERT INTO cpass.cpass_t_parametro (chiave, valore, abilitata, riferimento, ambiente, note, ente_id)
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
    ('AMBIENTE','PROD', true,'PROVVEDIMENTO', 'STILO', 'ambiente di integrazione', 'REGP')
) AS tmp (chiave, valore, abilitata, riferimento, ambiente, note, ente)
JOIN cpass.cpass_t_ente te ON te.ente_codice = tmp.ente 
WHERE NOT EXISTS (
  SELECT 1
