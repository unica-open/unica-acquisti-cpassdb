...
update cpass.cpass_t_parametro set valore = 'http://131.1.220.215/WSLoginCollaudo/rest' where chiave='MIT_URL_WSLOGIN';
update cpass.cpass_t_parametro set valore = 'http://131.1.220.215/WSProgrammiCollaudo/rest' where chiave='MIT_URL_WSPROGRAMMI';
...
SELECT tmp.chiave, tmp.valore, tmp.abilitata, tmp.riferimento, tmp.ambiente, tmp.note, te.ente_id
FROM (VALUES
    ('AMBIENTE','TEST', true,'PROVVEDIMENTO', 'STILO', 'ambiente di integrazione', 'REGP')
) AS tmp (chiave, valore, abilitata, riferimento, ambiente, note, ente)
JOIN cpass.cpass_t_ente te ON te.ente_codice = tmp.ente 
WHERE NOT EXISTS (
  SELECT 1
