...
-- PATCH_1.5.0
update cpass.cpass_t_parametro set valore = 'http://stilo.wsbe.regione.piemonte.it/stilobusiness/soap/WSTrovaDocFolder?wsdl' 
, ente_id = (select ente_id from cpass_t_ente where ente_codice = 'REGP')
where chiave = 'WSDL_LOCATION_RICERCA_DOC';
...
FROM ( VALUES
    ('AD', 'Atto dirigenziale', 'PROVVEDIMENTO', 'STILO', 'Per interrogazione Provvedimento', true, 'REGP')
    ,('DD', 'Determina dirigenziale', 'PROVVEDIMENTO', 'STILO', 'Per interrogazione Provvedimento', true, 'REGP')
    ,('DG', 'Delibera di giunta', 'PROVVEDIMENTO', 'STILO', 'Per interrogazione Provvedimento', true, 'REGP')
