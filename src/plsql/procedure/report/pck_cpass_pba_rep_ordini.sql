...
CREATE OR REPLACE FUNCTION cpass.pck_cpass_pba_rep_ordini(
  p_ordine_id character varying
)
RETURNS TABLE(
  ordine_anno integer,
  ordine_numero integer,
  ordine_consegna_da timestamp without time zone,
  ordine_consegna_a timestamp without time zone,
...
where tord.testata_ordine_id = p_ordine_id::UUID;
